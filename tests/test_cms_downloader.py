...
class TestFileCheckingMethods:
    ...
    def test_check_existing_jars_with_pricers(self, tmp_path):
        ...
        expected_pricers = {'ipps-pricer-application-2.10.0.jar', 'opps-pricer-application-2.11.0.jar'}
    ...
    def test_get_missing_jars_for_component_slf4j_all_present(self, tmp_path):
        ...
        existing_jars = {'main': {'slf4j-simple-2.0.9.jar', 'slf4j-api-2.0.9.jar'}, 'pricers': set()}
        missing = downloader.get_missing_jars_for_component('slf4j', existing_jars)
    ...
    def test_get_missing_jars_for_component_pricers(self, tmp_path):
        ...
        existing_jars = {'main': set(), 'pricers': {'ipps-pricer-application-2.10.0.jar'}}
    ...
    def test_get_all_missing_jars_empty_environment(self, tmp_path):
        ...
        with patch.object(downloader, 'check_existing_jars', return_value={'main': set(), 'pricers': set()}):
    ...
    def test_get_all_missing_jars_partial_environment(self, tmp_path):
        ...
        existing_jars = {'main': {'gfc-base-api-3.4.9.jar'}, 'pricers': set()}
        with patch.object(downloader, 'check_existing_jars', return_value=existing_jars):
            missing = downloader.get_all_missing_jars()
...
class TestUtilityMethods:
    ...
    def test_list_jar_inventory_empty(self, tmp_path):
        ...
        assert inventory['summary']['components_missing'] == len(downloader.REQUIRED_JARS)
    ...
    def test_list_jar_inventory_partial(self, tmp_path):
        ...
        existing_jars = {'main': {'gfc-base-api-3.4.9.jar', 'slf4j-simple-2.0.9.jar'}, 'pricers': set()}
        with patch.object(downloader, 'check_existing_jars', return_value=existing_jars):
            inventory = downloader.list_jar_inventory()
    ...
    def test_validate_jar_environment_incomplete(self, tmp_path):
        ...
        missing_components = {'slf4j': ['slf4j-api-2.0.9.jar'], 'gfc': ['gfc-base-api-3.4.9.jar']}
        with patch.object(downloader, 'get_all_missing_jars', return_value=missing_components):
            validation = downloader.validate_jar_environment()
    ...
    def test_map_url_to_jar_filename(self, tmp_path):
        ...
        test_cases = [
            ("https://www.cms.gov/files/zip/snf-pricer-20250-v241-executable-jar.zip", "snf-pricer-application-2.4.1.jar"),
            ("https://www.cms.gov/files/zip/esrd-pricer-20252-v280-executable-jar.zip", "esrd-pricer-application-2.8.0.jar"),
            ("https://www.cms.gov/files/zip/fqhc-pricer-20252-v270-executable-jar.zip", "fqhc-pricer-application-2.7.0.jar"),
            ("https://www.cms.gov/files/zip/hha-pricer-20251-v250-executable-jar.zip", "hha-pricer-application-2.5.0.jar"),
            ("https://www.cms.gov/files/zip/hospice-pricer-20250-v240-executable-jar.zip", "hospice-pricer-application-2.4.0.jar"),
            ("https://www.cms.gov/files/zip/ipps-pricer-20251-v2100-executable-jar.zip", "ipps-pricer-application-2.10.0.jar"),
            ("https://www.cms.gov/files/zip/ipf-pricer-20251-v250-executable.zip", "ipf-pricer-application-2.5.0.jar"),
            ("https://www.cms.gov/files/zip/irf-pricer-20250-v250-executable-jar.zip", "irf-pricer-application-2.5.0.jar"),
            ("https://www.cms.gov/files/zip/ltch-pricer-20251-v250-executable-jar.zip", "ltch-pricer-application-2.5.0.jar"),
            ("https://www.cms.gov/files/zip/opps-pricer-20253-v2110-executable-jar.zip", "opps-pricer-application-2.11.0.jar"),
        ]
        
        for url, expected_jar in test_cases:
            result = downloader.map_url_to_jar_filename(url)
            assert result == expected_jar, f"Failed for URL: {url}, expected {expected_jar}, got {result}"
...
class TestDownloadLogic:
    ...
    def test_process_slf4j_jar_force_download(self, tmp_path):
        ...
        with patch.object(downloader, 'is_component_complete', return_value=True):
            with patch.object(downloader, 'download_slf4j_jar', return_value=mock_paths):
    ...
    def test_process_slf4j_jar_missing_jars(self, tmp_path):
        ...
        with patch.object(downloader, 'is_component_complete', return_value=False):
            with patch.object(downloader, 'download_slf4j_jar', return_value=mock_paths) as mock_download:
    ...
    def test_process_grpc_jar_missing_jars(self, tmp_path):
        ...
        with patch.object(downloader, 'is_component_complete', return_value=False):
            with patch.object(downloader, 'download_grpc_jar', return_value=mock_paths) as mock_download:
                # Create the mock downloaded files
                (tmp_path / "downloads").mkdir()
                for path in mock_paths:
                    Path(path).touch()
            ...
            with patch.object(downloader, 'get_all_missing_jars', return_value={}):
                with patch.object(downloader, 'download_msdrg_files') as mock_msdrg:
                    with patch.object(downloader, 'download_ioce_files') as mock_ioce:
                        with patch.object(downloader, 'download_web_pricers') as mock_pricers:
                            
                            result = downloader.build_jar_environment(clean_existing=False)
    ...
    def test_process_zip_for_jars_selective_extraction(self, tmp_path):
        ...
        downloader.process_zip_for_jars(str(zip_path), "test", missing_jars=missing_jars)
