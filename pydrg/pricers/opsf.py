...
class OPSFDatabase:
    ...
    def create_table(self):
        columns = ", ".join([f"{key} {value['type']}" for key, value in DATATYPES.items()])
        ...
        self.cursor.execute("CREATE INDEX IF NOT EXISTS idx_opsf_provider_effective ON opsf (provider_ccn, effective_date)")
        #Create index on national_provider_identifier for faster lookups
        self.cursor.execute("CREATE INDEX IF NOT EXISTS idx_opsf_npi ON opsf (national_provider_identifier, effective_date)")
...
class OPSFProvider(BaseModel):
    ...
    def from_sqlite(self, conn, provider: Provider, date_int: int):
        ...
        if row:
            ...
            raise ValueError(f"No OPSF data found for provider {provider.other_id or provider.npi} on date {date_int}.")